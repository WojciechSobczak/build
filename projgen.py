...
def replace_launcher_variables(text: str, script_name: str, project_path: str, package_manager: str):
    text = text.replace("${script_name}", script_name)
    text = text.replace("${project_path}", project_path)
    text = text.replace("${package_manager}", package_manager)
    return text

def generate_python_executor(script_name: str, project_path: str, package_manager: str) -> str:
    with open(f"{_BASE_PROJGEN_FILES_PATH}/build.py", "r", encoding="utf-8") as file:
        text = file.read()
    return replace_launcher_variables(text, script_name, project_path, package_manager)

def generate_batch_executor(script_name: str, project_path: str, package_manager: str) -> str:
    with open(f"{_BASE_PROJGEN_FILES_PATH}/build.bat", "r", encoding="utf-8") as file:
        text = file.read()
    return replace_launcher_variables(text, script_name, project_path, package_manager)

def generate_shell_executor(script_name: str, project_path: str, package_manager: str) -> str:
...
def generate_script_launcher(launcher: Launcher, project_path: str, package_manager: str) -> str:
    script_name = commons.realpath(f"{os.path.dirname(__file__)}/build.py")
    project_path = commons.realpath(project_path)

    match launcher:
        case Launcher.PYTHON: return generate_python_executor(script_name, project_path, package_manager)
        case Launcher.BATCH: return generate_batch_executor(script_name, project_path, package_manager) 
        case Launcher.SHELL: return generate_shell_executor(script_name, project_path, package_manager)
...
def generate_project(package_manager: str, output_directory: str):
    ...
    with open(f"{output_directory}/build.py", "w", encoding="utf8") as file:
        file.write(generate_script_launcher(Launcher.PYTHON, output_directory, package_manager))

    with open(f"{output_directory}/build.sh", "w", encoding="utf8") as file:
        file.write(generate_script_launcher(Launcher.SHELL, output_directory, package_manager))
    
    with open(f"{output_directory}/build.bat", "w", encoding="utf8") as file:
        file.write(generate_script_launcher(Launcher.BATCH, output_directory, package_manager))
