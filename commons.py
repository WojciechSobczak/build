...
def _error_log_and_throw(msg: str):
    log.error(msg)
    raise Exception(msg)
...
def delete_dir(dir_path: str):
    if os.path.isdir(dir_path) == False:
        _error_log_and_throw(f"delete_dir(): {dir_path} is not a directory")
...
def rename_dir(source_dir_path: str, target_dir_name: str):
    if os.path.isdir(source_dir_path) == False:
        _error_log_and_throw(f"rename_dir(): source_dir_path: {source_dir_path} is not a directory")

    # Some checks to avoid problems, not full check as no idea how
    if ('/' in target_dir_name) or ('\\' in target_dir_name):
        _error_log_and_throw(f"rename_dir(): target_dir_name: {target_dir_name} must be a plain name")
